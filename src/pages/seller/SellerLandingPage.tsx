...
const PaymentFlowAnimation = () => {
    ...
    const IS_PAYOUT = active === SELLER_STEPS.length - 1;

    useEffect(() => {
        intervalRef.current = setInterval(() => {
            setActive(prev => {
                const next = (prev + 1) % SELLER_STEPS.length;
                if (next === SELLER_STEPS.length - 1) setCelebKey(k => k + 1);
                return next;
            });
        }, STEP_DURATION);
    ...
    const step = SELLER_STEPS[active];
    const progressPct = ((active + 1) / SELLER_STEPS.length) * 100;

    return (
        <div className="mb-20">
            <style>{SUCCESS_ANIMATION_KEYFRAMES}</style>
            <p className="text-center text-sm text-gray-400 mb-10 font-medium italic">Watch your business flow animate automatically ↓</p>
            ...
            <div className="relative">
                <div className="hidden md:block absolute top-10 left-[8.33%] right-[8.33%] h-0.5 bg-gray-100 z-0">
                    <div
                        ...
                        style={{
                            left: `${Math.min((active / (SELLER_STEPS.length - 1) + (active === SELLER_STEPS.length - 1 ? 0 : dotPct / (SELLER_STEPS.length - 1))) * 100, 100)}%`,
                            background: step.colors.badge,
                            boxShadow: `0 0 8px 3px ${step.colors.glow}`,
                            transform: 'translate(-50%, -50%)',
                            opacity: active === SELLER_STEPS.length - 1 && dotPct > 0.5 ? 0 : 1
                        }}
                    />
                    <div
                        className="absolute inset-y-0 left-0 rounded-full transition-all duration-500"
                        style={{ width: `${(active / (SELLER_STEPS.length - 1)) * 100}%`, background: step.colors.badge }}
                    />
                </div>
                ...
                <div className="grid grid-cols-3 md:grid-cols-6 gap-3 md:gap-4 relative z-10">
                    {SELLER_STEPS.map((s, i) => {
                        ...
                        const isPayout = i === SELLER_STEPS.length - 1;

                        return (
                            <button
                                key={s.step}
                                onClick={() => {
                                    if (isPayout && !isActive) setCelebKey(k => k + 1);
                                    setActive(i);
                            ...
                            >
                                ...
                                >
                                    ...
                                    {isPayout && isActive && <SuccessAnimation key={celebKey} />}
                                </div>
                                <span
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                        <div className="flex-1">
                            <div className="flex items-center gap-3 mb-3">
                                <span className={`text-[10px] font-black px-2.5 py-1 rounded-full uppercase tracking-widest ${IS_PAYOUT ? 'bg-white/20 text-white' : 'bg-gray-900 text-white'}`}>
                                    Step {step.step}
                                </span>
                                {IS_PAYOUT && (
                            ...
                            <h3 className={`text-2xl md:text-4xl font-black mb-3 ${IS_PAYOUT ? 'text-white' : 'text-gray-900'}`}>{step.label}</h3>
                            <p className={`text-base md:text-lg max-w-xl ${IS_PAYOUT ? 'text-teal-50' : 'text-gray-600'}`}>{step.desc}</p>
                        </div>

                        <div className="flex items-center gap-4">
                            <div className={`w-1 h-12 rounded-full ${IS_PAYOUT ? 'bg-white/20' : 'bg-gray-100'}`} />
                            <div className="text-right flex flex-col items-end">
                                ...
                                <span className={`text-xs font-black uppercase tracking-tighter ${IS_PAYOUT ? 'text-teal-100' : 'text-gray-400'}`}>Business Workflow: Live</span>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div className="min-h-screen bg-white font-sans text-gray-900">
            ...
            <section className="py-24 bg-blue-900 text-white relative overflow-hidden">
                ...
                <div className="max-w-4xl mx-auto px-4 text-center relative z-10">
                    <h2 className="text-3xl md:text-5xl font-black mb-6 tracking-tight">Ready to grow your business?</h2>
                    <p className="text-xl text-blue-100 mb-10 font-medium">Join hundreds of print shops already benefiting from QuickXerox.</p>
                    <a
                        href="mailto:partner@quickxerox.app"
                        className="w-full md:w-auto px-10 py-5 bg-white text-blue-900 text-lg font-black rounded-full hover:bg-gray-100 transition-all shadow-xl hover:shadow-2xl transform hover:-translate-y-1 inline-block"
                    >
            ...
            <footer className="bg-gray-950 text-white py-12 border-t border-gray-900 text-center">
                <div className="max-w-7xl mx-auto px-4">
                    <p className="text-gray-500 mb-4 font-medium">&copy; {new Date().getFullYear()} QuickXerox Partner. All rights reserved.</p>
                    <div className="flex justify-center gap-8 text-gray-400 text-sm font-bold">
