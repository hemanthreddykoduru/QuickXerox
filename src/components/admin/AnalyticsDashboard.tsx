...
const AnalyticsDashboard: React.FC<AnalyticsDashboardProps> = ({ onManageUsers, onManageShops, onViewOrders }) => {
  ...
  const [topShopsData, setTopShopsData] = useState<{ name: string; orders: number; revenue: number; }[]>([]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      const unsubShops = onSnapshot(shopsRef, (snap) => {
        ...
        // Build shop names map
        const namesMap: Record<string, string> = {};
        snap.docs.forEach(doc => {
          const data = doc.data() as any;
          const shopName = data.settings?.shop?.name || data.shopName || data.name || data.businessName || `Shop ${doc.id.substring(0, 6)}`;
          namesMap[doc.id] = shopName;
      ...
      const unsubOrders = onSnapshot(query(ordersRef, orderBy('timestamp', 'desc')), (snap) => {
        ...
        // Aggregate orders by shop to find top performing shops
        ...
        orders.forEach(o => {
          ...
          if (!shopStats[shopId]) {
            ...
            const mappedName = shopNamesMap[shopId];
            const finalName = mappedName || o.sellerName || o.shopName || `Shop ${shopId.substring(0, 6)}`;
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        {topShopsData.length > 0 ? (
          ...
          <div className="h-[300px] flex items-center justify-center text-gray-500">
            <p>No shop performance data available yet</p>
          </div>
        )}
      </div>
